...
class UserService extends BaseService {
  ...
  async loginUser(req) {
    try {
      ...
      const adminExists = await UserModel.findOne({email: post.email, userType: 'admin'})

      if(adminExists && adminExists.userType == 'admin'){
        if(adminExists.deviceName !== deviceInfo.deviceName){
          return BaseService.sendFailedResponse({error: `An attempt to login in as an admin in another (${deviceInfo.deviceName}) device detected!`})
        }else{
          ...
          const token = await adminExists.generateToken(process.env.TOKEN_SECRET || 'enter_your_key!.')
          return BaseService.sendSuccessResponse({
            message: token,
          });
        }
      ...
      if(!userExists.userType){
        return BaseService.sendFailedResponse({error: 'You have not completed your profile.'})
      }
  ...
  async googleAuthEmail(req, res) {
    try {
      ...
      if(empty(userExists)){
        ...
        // }
        if(!userExists.userType){
          return BaseService.sendFailedResponse({error: 'incomplete-registration'})
        }
        const token = await userExists.generateToken(process.env.TOKEN_SECRET || 'enter_your_key!.')
        return BaseService.sendSuccessResponse({message: token, google_type: 'login', userType: userExists.userType})
    }
  ...
  async registerUser(req, res) {
    ...
    try {
      ...
      if(empty(userExists)){
        return BaseService.sendFailedResponse({error: 'User does not exist. Please register!'})
      }

      if(post.userType == 'vendor' && (empty(post.category) || empty(post.subcategories))){
        return BaseService.sendFailedResponse({error: 'Please provide your category and subcategory'})
      }
